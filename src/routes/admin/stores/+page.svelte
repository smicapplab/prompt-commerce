<script lang="ts">
  import { Plus, Pencil, Trash2, Copy, Check, ToggleLeft, ToggleRight, Store, Search, Loader } from 'lucide-svelte';

  interface StoreItem {
    id: number; slug: string; name: string; description: string | null;
    gateway_key: string | null; active: number; created_at: string;
  }
  ...
  let looked = $state<{ slug: string; name: string; mcpServerUrl: string } | null>(null);
  ...
  function openEdit(s: StoreItem) {
    editTarget = s;
    form = { slug: s.slug, name: s.name, description: s.description ?? '', gateway_key: s.gateway_key ?? '' };
    showModal = true;
  }
  ...
  async function lookupKey() {
    ...
    const res = await fetch(`/api/stores/lookup?key=${encodeURIComponent(keyInput.trim())}`, {
  ...
  async function deleteStore(s: StoreItem) {
    if (!confirm(`Delete "${s.name}"? This will also delete all its products, orders and conversations.`)) return;
    const res = await fetch(`/api/stores/${s.id}`, {
      method: 'DELETE',
      headers: { Authorization: `Bearer ${token()}` },
    });
    if (res.ok) { showToast('Store deleted'); load(); }
...
<!-- Toast -->
{#if toast}
  <div class="fixed top-4 right-4 z-50 px-4 py-3 rounded-lg text-sm font-medium shadow-lg
...
<div class="p-6">
  <div class="flex items-center justify-between mb-6">
    <div>
      ...
      <p class="text-sm text-gray-500 mt-0.5">Manage stores hosted on this server</p>
    </div>
    <button
      onclick={openCreate}
      ...
      <table class="w-full text-sm">
        <thead class="bg-gray-50 border-b border-gray-200">
          <tr>
            ...
            <th class="text-left px-4 py-3 font-medium text-gray-600">Gateway Key</th>
        ...
        <tbody class="divide-y divide-gray-100">
          {#each stores as s}
            <tr class="hover:bg-gray-50">
              ...
              <td class="px-4 py-3">
                {#if s.gateway_key}
                  <div class="flex items-center gap-2">
                    <span class="font-mono text-xs text-gray-500">{s.gateway_key.slice(0, 12)}…</span>
                    <button onclick={() => copyKey(s)} class="text-gray-400 hover:text-gray-600">
                      {#if copiedKey === s.id}<Check class="w-3.5 h-3.5 text-green-500" />{:else}<Copy class="w-3.5 h-3.5" />{/if}
                    </button>
                  </div>
                {:else}
              ...
              <td class="px-4 py-3">
                <button onclick={() => toggleActive(s)} class="flex items-center gap-1.5 text-xs">
                  {#if s.active}
              ...
              <td class="px-4 py-3">
                <div class="flex items-center gap-1 justify-end">
                  <button onclick={() => openEdit(s)} class="p-1.5 rounded hover:bg-gray-100 text-gray-500">
                    <Pencil class="w-4 h-4" />
                  </button>
                  <button onclick={() => deleteStore(s)} class="p-1.5 rounded hover:bg-red-50 text-gray-500 hover:text-red-600">
  ...
  <div class="fixed inset-0 bg-black/40 z-40 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-xl w-full max-w-md">
      <div class="px-6 py-4 border-b border-gray-100">
        <h2 class="font-semibold text-gray-900">{editTarget ? 'Edit store' : 'Connect store'}</h2>
      </div>
      ...
      {#if editTarget}
        ...
        <div class="px-6 py-4 space-y-4">
          <div>
            <label for="edit-name" class="block text-sm font-medium text-gray-700 mb-1">Store name *</label>
            <input id="edit-name"
          ...
          <div>
            <label for="edit-key" class="block text-sm font-medium text-gray-700 mb-1">Gateway key</label>
            <input id="edit-key"
        ...
        <div class="px-6 py-4 border-t border-gray-100 flex justify-end gap-3">
          <button onclick={() => (showModal = false)} class="px-4 py-2 text-sm text-gray-600 hover:text-gray-900">Cancel</button>
          <button
            onclick={saveStore}
      ...
      {:else}
        ...
        <div class="px-6 py-5 space-y-4">
          ...
          <div>
            <label for="add-key" class="block text-sm font-medium text-gray-700 mb-1">Platform key</label>
            <div class="flex gap-2">
              <input id="add-key"
                bind:value={keyInput}
                onkeydown={(e) => { if (e.key === 'Enter') lookupKey(); }}
                type="text" placeholder="gk_…"
            ...
            <p class="text-xs text-gray-400 mt-1">Paste the key issued by the gateway admin.</p>
          </div>
          ...
          {#if lookupState === 'found' && looked}
            <div class="rounded-lg bg-green-50 border border-green-200 px-4 py-3 space-y-2">
              <p class="text-xs font-medium text-green-700 uppercase tracking-wide">Store found</p>
              <div class="flex items-baseline justify-between">
                <span class="text-sm font-semibold text-gray-900">{looked.name}</span>
                <span class="text-xs font-mono text-gray-500">{looked.slug}</span>
              </div>
              <p class="text-xs text-gray-400 truncate">{looked.mcpServerUrl}</p>
            </div>
          {/if}
