...
<div class="min-h-full flex flex-col items-center justify-center p-8">
  <div class="w-full max-w-lg">
    ...
    <div class="text-center mb-8">
      ...
      <p class="text-sm text-gray-500 mt-1">Choose which store you want to manage</p>
    </div>
    ...
    {:else if stores.length === 0}
      <div class="text-center py-12 border-2 border-dashed border-gray-200 rounded-2xl">
        ...
        <p class="text-xs text-gray-400 mt-1">Register a store via the gateway, then connect it here.</p>
        <a
          href="/admin/stores"
    ...
    {:else}
        ...
        {#each stores.filter(s => s.active) as s}
          ...
          >
            ...
            <div class="flex items-center gap-2 flex-shrink-0">
              {#if !s.gateway_key}
                <span class="text-xs text-amber-600 bg-amber-50 px-2 py-0.5 rounded-full">No key</span>
              {/if}
              <ArrowRight class="w-4 h-4 text-gray-300 group-hover:text-blue-500 transition-colors" />
            </div>
          </button>
        {/each}

        {#each stores.filter(s => !s.active) as s}
          <div class="w-full flex items-center gap-4 bg-gray-50 border border-gray-200 rounded-xl px-5 py-4 opacity-50">
            <div class="w-10 h-10 rounded-xl bg-gray-100 flex items-center justify-center flex-shrink-0">
              <Store class="w-5 h-5 text-gray-400" />
            </div>
            <div>
              <p class="font-medium text-gray-600 text-sm">{s.name}</p>
              <p class="text-xs text-gray-400 font-mono mt-0.5">{s.slug} · Inactive</p>
            </div>
          </div>
        {/each}
      </div>
      ...
      <div class="mt-6 text-center">
        <a href="/admin/stores" class="text-sm text-gray-500 hover:text-gray-700">
          Manage stores →
        </a>
      </div>
    {/if}
