...
<div class="space-y-8 animate-in fade-in duration-500 pb-20">
  ...
  <div>
    ...
    <p class="text-sm text-gray-500 mt-1">Configure the core connectivity between the seller panel and the AI gateway.</p>
  </div>
    ...
    <div class="bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-shadow">
      ...
      <div class="p-6 space-y-4 flex-1">
        <div class="space-y-1.5 transition-all focus-within:text-indigo-600">
           <label for="g-url" class="text-[11px] font-black uppercase tracking-widest px-1">Gateway URL</label>
           <div class="relative group">
              <Link2 size={14} class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-indigo-500" />
              <input
                id="g-url"
                type="text"
                disabled={userRole !== "super_admin"}
                value={serverSettings.gateway_url}
                oninput={(e) => setServer("gateway_url", (e.target as HTMLInputElement).value)}
           ...
           <p class="text-[10px] text-gray-400 leading-tight">Must match the base URL of your NestJS gateway instance.</p>
        </div>
    ...
    <div class="bg-white rounded-3xl border border-gray-200 shadow-sm overflow-hidden flex flex-col hover:shadow-md transition-shadow">
      ...
      <div class="p-6 space-y-4 flex-1">
         <div class="space-y-1.5 transition-all focus-within:text-blue-600">
            <label for="p-url" class="text-[11px] font-black uppercase tracking-widest px-1">Seller Public URL</label>
            <input
              id="p-url"
              type="text"
              disabled={userRole !== "super_admin"}
              value={serverSettings.seller_public_url}
              oninput={(e) => setServer("seller_public_url", (e.target as HTMLInputElement).value)}
            ...
            />
            <p class="text-[10px] text-gray-400 leading-tight">Required for canonical image URLs and customer callbacks.</p>
         </div>
  ...
  <!-- Actions -->
  {#if userRole === "super_admin"}
    <div class="sticky bottom-0 bg-white/90 backdrop-blur-md border-t border-gray-100 p-6 -mx-6 flex items-center justify-between z-10">
      <button
        onclick={saveServer}
        disabled={saving}
