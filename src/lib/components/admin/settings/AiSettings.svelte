...
{#if !activeStore.slug}
  <div class="flex flex-col items-center justify-center py-12 text-center">
    ...
    <p class="text-sm text-gray-500 max-w-xs mt-1">
      AI settings are store-specific. Select a store to begin.
    </p>
  </div>
{:else}
  <div class="space-y-8 animate-in fade-in duration-500 pb-20">
    ...
    <div>
      ...
      <p class="text-sm text-gray-500 mt-1">
        Configure the brain behind your automated storefront.
      </p>
    </div>
        ...
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <p class="text-xs text-gray-500">
              Pick your preferred LLM and provider.
            </p>
          </div>
        </div>
      </div>
        ...
        <div class="space-y-3">
          ...
          {#if customModelMode}
            <div class="animate-in slide-in-from-top-2 duration-300">
              ...
              />
              <p class="mt-1 text-[10px] text-purple-600 font-medium">
                Use the provider's exact model ID string.
              </p>
            </div>
          {/if}
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="space-y-6">
        ...
        >
          ...
          <div class="relative">
            ...
            >
              {#if showClaudeKey}<EyeOff size={14} />{:else}<Eye
                  size={14}
                />{/if}
            </button>
          </div>
        ...
        >
          ...
          <div class="relative">
            ...
            <button
              ...
              class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-300 hover:text-gray-500"
            >
              {#if showGeminiKey}<EyeOff size={14} />{:else}<Eye
                  size={14}
                />{/if}
            </button>
          </div>
        ...
        >
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
              <div
                class="w-8 h-8 rounded-lg bg-teal-100 flex items-center justify-center text-teal-600"
              >
                <Search size={16} />
              </div>
          ...
          <div class="relative">
            ...
            <button
              ...
              class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-300 hover:text-gray-500"
            >
              {#if showSerperKey}<EyeOff size={14} />{:else}<Eye
                  size={14}
                />{/if}
            </button>
          </div>
          <p class="text-[10px] text-gray-400">
            Used for real-time web search capabilities in chat.
          </p>
        </div>
      </div>
      ...
      <div
        class="flex flex-col bg-white rounded-2xl border border-gray-200 p-6 space-y-4"
      >
        <label
          for="ai-prompt"
        ...
        <p class="text-[11px] text-gray-400 leading-tight">
          Define how the AI should behave, what it knows about your brand, and
          any specific selling guidelines.
        </p>
      </div>
    </div>
    ...
    <!-- MCP Section -->
    <div
      class="bg-indigo-900 rounded-2xl p-6 text-white shadow-xl shadow-indigo-100 border border-indigo-800"
    >
      <div class="flex items-center gap-3 mb-6">
        <div
          class="w-10 h-10 rounded-xl bg-indigo-800 flex items-center justify-center text-indigo-400 border border-indigo-700"
        >
          <ShieldCheck size={24} />
        </div>
        <div>
          ...
          <p class="text-indigo-400 text-xs mt-1">
            Connect this store to external AI tools (Claude Desktop, etc.)
          </p>
        </div>
      </div>
      ...
      {#if tempMcpKey}
        <div
          class="space-y-4 p-4 rounded-xl bg-indigo-950/50 border border-indigo-700 animate-in zoom-in duration-300"
        >
          <div class="relative">
            <input
              type="text"
          ...
          <div class="flex items-center justify-between text-[11px] font-bold">
            <span class="text-indigo-400 uppercase tracking-widest"
              >Expires: {new Date(tempMcpExpiry).toLocaleTimeString()}</span
            >
      ...
      {:else}
        <div class="flex items-center gap-4">
          <div class="flex-1 space-y-1.5">
            <label
              for="duration"
              class="text-[10px] font-black text-indigo-400 uppercase tracking-widest px-1"
              >Duration</label
            >
            <select
              id="duration"
    ...
    >
      <div class="flex items-center gap-3">
        ...
        {#if aiGatewayStatus === "synced"}
          <div
            class="px-4 py-2 rounded-xl bg-green-50 text-green-700 border border-green-100 flex items-center gap-2 shadow-sm animate-in fade-in slide-in-from-left-4"
          >
            <div
        ...
        {:else if aiGatewayStatus === "failed"}
          ...
          >
            ...
            <span class="text-[11px] font-black uppercase tracking-tight"
              >Sync Failed: {aiGatewayReason}</span
            >
          </div>
        {/if}
      </div>
