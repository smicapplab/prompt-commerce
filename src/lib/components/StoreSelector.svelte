<script lang="ts">
  ...
  async function loadStores() {
    ...
    selectedSlug =
      stores.find((s) => s.slug === saved)?.slug ?? stores[0]?.slug ?? null;
    ...
    if (selectedSlug) {
      ...
      document.dispatchEvent(
        new CustomEvent("store-changed", {
          detail: { slug: s.slug, id: s.id },
        }),
      );
    }
  }
  ...
  function select(s: Store) {
    ...
    open = false;
    document.dispatchEvent(
      new CustomEvent("store-changed", { detail: { slug: s.slug, id: s.id } }),
    );
  }
